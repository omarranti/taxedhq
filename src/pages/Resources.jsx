...
const C = { primary: "#1B4D3E", accent: "#F59E0B", bg: "#FAFAF8", surface: "#FFFFFF", border: "#E5E5E0", text: "#1A1A1A", textSec: "#6B6B6B" };
...
function ResourceCard({ icon: Icon, title, desc, link, linkText = "Read Guide" }) {
    return (
        <div style={{ background: C.surface, padding: 24, borderRadius: 20, border: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background: `${C.primary}10`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon size={22} color={C.primary} /></div>
            <div>
            ...
            <a href={link} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginTop: 'auto', paddingTop: 16, color: C.primary, fontSize: 14, fontWeight: 700, textDecoration: 'none', borderTop: `1px solid ${C.border}` }}>{linkText} <ArrowRight size={16} /></a>
        </div>
    );
}
        ...
        <div style={{ fontFamily: font.sans, background: C.bg, minHeight: '100vh', padding: '60px 24px 120px' }}>
            <div style={{ maxWidth: 1200, margin: '0 auto 60px', textAlign: 'center' }}>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 16px', background: `${C.accent}15`, borderRadius: 99, color: '#D97706', fontSize: 12, fontWeight: 700, letterSpacing: '0.04em', textTransform: 'uppercase', marginBottom: 24 }}>Open Directory</div>
                    <h1 style={{ fontFamily: font.serif, fontSize: 'clamp(36px, 6vw, 56px)', color: C.text, lineHeight: 1.1, marginBottom: 20 }}>Everything you need. <br />Nothing you don't.</h1>
                    <p style={{ fontSize: 18, color: C.textSec, maxWidth: 600, margin: '0 auto' }}>A curated collection of plain-English guides, essential IRS links, and essential tax deadlines.</p>
                </motion.div>
            </div>
            <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 60 }}>
                <section>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}><Database size={24} color={C.primary} /><h2 style={{ fontFamily: font.serif, fontSize: 28, color: C.text }}>Interactive Tools</h2></div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: 20 }}>
                        <div style={{ padding: 32, background: `linear-gradient(135deg, ${C.primary}, #0A261D)`, borderRadius: 24, color: '#fff', position: 'relative', overflow: 'hidden' }}>
                            ...
                            <h3 style={{ fontFamily: font.serif, fontSize: 24, marginBottom: 16, position: 'relative', zIndex: 1 }}>Tax Clarity Platform</h3>
                            ...
                            <Link to="/calculator" style={{ padding: '12px 24px', background: C.accent, color: '#000', borderRadius: 99, fontSize: 14, fontWeight: 700, textDecoration: 'none', display: 'inline-block', position: 'relative', zIndex: 1 }}>Launch Tax Navigator</Link>
                        </div>
                        <div style={{ padding: 32, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 24 }}>
                            ...
                            <p style={{ color: C.textSec, fontSize: 15, lineHeight: 1.6, marginBottom: 24 }}>Use Taxed to understand your situation first, then connect with a verified tax professional to handle your actual filing. Knowledge = better conversations.</p>
                ...
                <section>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}><BookOpen size={24} color={C.primary} /><h2 style={{ fontFamily: font.serif, fontSize: 28, color: C.text }}>Essential Guides</h2></div>
                ...
                <section>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}><Calendar size={24} color={C.primary} /><h2 style={{ fontFamily: font.serif, fontSize: 28, color: C.text }}>2024 Deadlines</h2></div>
                    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 20, overflow: 'hidden' }}>
                        {[
                            ...
                            <div key={i} style={{ display: 'flex', gap: 24, padding: 24, borderBottom: i < 2 ? `1px solid ${C.border}` : 'none' }}>
                                ...
                                <div><div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 8 }}>{d.title}</div><div style={{ fontSize: 14, color: C.textSec, lineHeight: 1.5 }}>{d.desc}</div></div>
                            </div>
                        ))}
                    </div>
                </section>
