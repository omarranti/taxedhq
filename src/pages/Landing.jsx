...
const C = {
    primary: "#1B4D3E", primaryLight: "#2D7A5F", accent: "#F59E0B", bg: "#FAFAF8",
    surface: "#FFFFFF", border: "#E5E5E0", text: "#1A1A1A", textSec: "#6B6B6B",
    success: "#10B981", muted: "#9CA3AF"
};
const font = { serif: "'DM Serif Display', Georgia, serif", sans: "'DM Sans', system-ui, sans-serif" };
const fadeUp = { hidden: { opacity: 0, y: 24 }, show: { opacity: 1, y: 0, transition: { duration: 0.6, ease: [0.25, 0.1, 0.25, 1] } } };
    ...
    return (
        <div style={{ fontFamily: font.sans, background: C.bg }}>
                ...
                <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 1 }}>
                    <motion.div initial="hidden" animate="show" variants={stagger}>
                        <motion.div variants={fadeUp} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 16px', background: `${C.primary}10`, borderRadius: 99, color: C.primary, fontSize: 13, fontWeight: 700, letterSpacing: '0.04em', textTransform: 'uppercase', border: `1px solid ${C.primary}25`, marginBottom: 32 }}>
                            <span style={{ width: 8, height: 8, borderRadius: '50%', background: C.primary }} /> Tax Clarity Platform
                        </motion.div>
                        ...
                        <motion.div variants={fadeUp} style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 24px', background: C.surface, borderRadius: 99, border: `1px solid ${C.border}`, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', flexWrap: 'wrap', justifyContent: 'center' }}>
                                <span style={{ fontFamily: font.serif, fontSize: 22, color: C.primary, fontWeight: 700 }}>$29.99</span>
            ...
            <section id="how-it-works" style={{ padding: '100px 24px' }}>
                <div style={{ maxWidth: 1200, margin: '0 auto' }}>
                    <div style={{ textAlign: 'center', marginBottom: 60 }}>
                        ...
                        <p style={{ fontSize: 18, color: C.textSec, maxWidth: 600, margin: '0 auto' }}>Stop blindly trusting a number on a screen. Taxed gives you the full picture — across every income level, scenario, and life change.</p>
                    </div>
            ...
            <section style={{ padding: '0 24px 100px' }}>
                <div style={{ maxWidth: 900, margin: '0 auto' }}>
                    <div style={{ textAlign: 'center', marginBottom: 48 }}>
                        ...
                        <p style={{ fontSize: 16, color: C.textSec }}>TurboTax files. CPAs file. Taxed <em>explains</em>.</p>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 16 }}>
                        {[
                            ...
                            <div key={i} style={{ padding: 28, borderRadius: 20, border: `2px solid ${col.highlight ? C.primary : C.border}`, background: col.highlight ? `${C.primary}04` : C.surface, position: 'relative' }}>
                                {col.highlight && (<div style={{ position: 'absolute', top: -14, left: '50%', transform: 'translateX(-50%)', background: C.primary, color: '#fff', fontSize: 11, fontWeight: 700, padding: '4px 14px', borderRadius: 99, whiteSpace: 'nowrap', letterSpacing: '0.04em', textTransform: 'uppercase' }}>The Gap We Fill</div>)}
                                ...
                                <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
                                    {col.items.map((item, j) => (<li key={j} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 14, color: col.highlight ? C.text : C.textSec }}><Check size={15} color={col.highlight ? C.success : C.muted} style={{ flexShrink: 0 }} /> {item}</li>))}
                                </ul>
                            </div>
                        ))}
                    </div>
                    ...
                    <div style={{ flex: 1, minWidth: 280, display: 'flex', flexDirection: 'column', gap: 16, position: 'relative', zIndex: 1 }}>
                        {["Income Level Navigation Guide", "How to Request IRS Penalty Abatement", "Credits You're Probably Missing"].map((t, i) => (
                            <div key={i} style={{ padding: 24, background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)', borderRadius: 16, border: '1px solid rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', gap: 16 }}>
                                <div style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><FileText size={20} color="#fff" /></div>
            ...
            <section style={{ padding: '100px 24px', textAlign: 'center' }}>
                ...
                <p style={{ marginTop: 16, fontSize: 13, color: C.muted }}>Educational tool only. Taxed does not file taxes or provide tax advice.</p>
            </section>
        </div>
    );
}
